...
<body>
    <div class="container" style=" margin-bottom: 20px;">
        ...
        <div class="content">
            ...
            <p style="margin-top: -8px;"><label>Nama</label><span class="titik_dua">:</span> <span>{{ $dispensasi->nama }}</span></p>
            <p style="margin-top: -11px;"><label>NIS</label><span class="titik_dua">:</span> <span>{{ Auth::user()->nis }}</span></p>
            <p style="margin-top: -11px;"><label>Kelas</label><span class="titik_dua">:</span> <span>{{ $dispensasi->tingkat }} {{ $dispensasi->konsentrasi_keahlian }}</span></p>
            <p style="margin-top: -11px;"><label>Program Keahlian</label><span class="titik_dua">:</span> <span>{{ $dispensasi->program_keahlian }}</span></p>
            <p style="margin-top: -11px;"><label>Untuk</label><span class="titik_dua">:</span> <span>{{ $dispensasi->kategori }}</span></p>
            <p style="margin-top: -11px;"><label>Mata Pelajaran</label><span class="titik_dua">:</span> <span>{{ $dispensasi->mata_pelajaran }}</span></p>
            <p style="margin-top: -11px;"><label>Waktu</label><span class="titik_dua">:</span> <span>{{ \Carbon\Carbon::parse($dispensasi->waktu_keluar)->format('d-m-Y, H:i:s') }}</span></p>
            <p style="margin-top: -11px;"><label>Alasan</label><span class="titik_dua">:</span> <span>{{ $dispensasi->alasan }}</span></p>
            <p style="margin-top: -11px;">Demikian surat izin ini dibuat agar yang berkepentingan mengetahui dan maklum adanya.</p>
        </div>
        @foreach($dispensasi->konfirmasi as $konfirm)
        <div class="signature-container">
            <div class="signature_1" style="width: 30%; text-align: center;">
                ...
                <br><br>
                <?php
                // Query untuk mendapatkan nama petugas berdasarkan NIP
                $guru = \App\Models\AkunGuru::where('nip', $konfirm->konfirmasi_3)->first();
                ?>
                <span>{{ $guru ? $guru->nama : 'Tidak ditemukan' }}</span>
            ...
            <div class="signature_2" style="text-align: center; margin: -180px auto; width: fit-content;">
                ...
                <?php
                // Query untuk mendapatkan nama petugas berdasarkan NIP
                $guru = \App\Models\AkunGuru::where('nip', $konfirm->konfirmasi_2)->first();
                ?>
                <span>{{ $guru ? $guru->nama : 'Tidak ditemukan' }}</span>
                <p style="margin-top: -15px;">________________________</p>
            ...
            <div class="signature_3" style="float: right; text-align: center; margin-top: -35px; margin-right: -20px;">
                ...
                <?php
                // Query untuk mendapatkan nama petugas berdasarkan NIP
                $guru = \App\Models\AkunGuru::where('nip', $konfirm->konfirmasi_1)->first();
                ?>
                <span>{{ $guru ? $guru->nama : 'Tidak ditemukan' }}</span>
    ...
    <div class="container" style="margin-top: 35px;">
        ...
        <div class="content">
            ...
            <p style="margin-top: -8px;"><label>Nama</label><span class="titik_dua">:</span> <span>{{ $dispensasi->nama }}</span></p>
            <p style="margin-top: -11px;"><label>NIS</label><span class="titik_dua">:</span> <span>{{ Auth::user()->nis }}</span></p>
            <p style="margin-top: -11px;"><label>Kelas</label><span class="titik_dua">:</span> <span>{{ $dispensasi->tingkatan }} {{ $dispensasi->konsentrasi_keahlian }}</span></p>
            <p style="margin-top: -11px;"><label>Program Keahlian</label><span class="titik_dua">:</span> <span>{{ $dispensasi->program_keahlian }}</span></p>
            <p style="margin-top: -11px;"><label>Untuk</label><span class="titik_dua">:</span> <span>{{ $dispensasi->kategori }}</span></p>
            <p style="margin-top: -11px;"><label>Mata Pelajaran</label><span class="titik_dua">:</span> <span>{{ $dispensasi->mata_pelajaran }}</span></p>
            <p style="margin-top: -11px;"><label>Waktu</label><span class="titik_dua">:</span> <span>{{ \Carbon\Carbon::parse($dispensasi->waktu_keluar)->format('d-m-Y, H:i:s') }}</span></p>
